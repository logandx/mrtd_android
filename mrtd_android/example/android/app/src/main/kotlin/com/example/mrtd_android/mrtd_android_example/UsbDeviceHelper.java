...
public class UsbDeviceHelper {
    ...
    public UsbDeviceHelper(Context context) {
        ...
        mPermissionIntent = PendingIntent.getBroadcast(mContext, 0, new Intent("com.hdos.usbdevice.UsbDeviceLib.USB_PERMISSION"), PendingIntent.FLAG_IMMUTABLE);
    ...
    public void ReleaseUsbDeviceHelper() {
        if (mContext != null) mContext.unregisterReceiver(mUsbReceiver);
    }
    ...
    private final BroadcastReceiver mUsbReceiver = new BroadcastReceiver() {
        public void onReceive(Context context, Intent intent) {
            ...
            if (action.equals(UsbManager.ACTION_USB_DEVICE_ATTACHED)) {
                ...
                Log.d(TAG, "UsbDevice (VID:" + usbDevice.getVendorId() + ",PID:" + usbDevice.getProductId() + ") DEVICE_ATTACHED");

                //requestUserPermission(usbDevice);

            } else if (action.equals(UsbManager.ACTION_USB_DEVICE_DETACHED)) {
                assert usbDevice != null;
                Log.d(TAG, "UsbDevice (VID:" + usbDevice.getVendorId() + ",PID:" + usbDevice.getProductId() + ") DEVICE_DETACHED");

            } else if (action.equals(ACTION_USB_DEVICE_PERMISSION) || action.equals("com.hdos.usbdevice.UsbDeviceLib.USB_PERMISSION")) {
                // 获取连接设备的权限
    ...
    private void requestUserPermission(UsbDevice usbDevice) {
        ...
        PendingIntent mPendingIntent = PendingIntent.getBroadcast(mContext, 0, new Intent(ACTION_USB_DEVICE_PERMISSION), PendingIntent.FLAG_IMMUTABLE);
    ...
    public boolean isOpen() {
        if ((mUsbManager != null) && (mUsbDevice != null) && (mDeviceConnection != null) && (mUsbManager.hasPermission(mUsbDevice))) {
            return true;
        }
        return false;
    }
